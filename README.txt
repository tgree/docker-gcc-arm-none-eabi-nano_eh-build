...
To achieve this reproducibility, the ARM build scripts have been modified to
use a fixed "release date" when building.  Normally, today's date would be used
when doing a new compiler build, however that generates a string that is
embedded in the compiler and later embedded in .comment sections of the
...
building the compiler the build scripts are patched to use the same date as
the original compiler build.  Furthermore, ARM has done something weird in
their release where the release date is in August but the release version is
in October - something that appears to be impossible using the present
scripts.  Anyways, to match this we specify a different release version
compared to the release date.
...
cd docker
docker build -t gcc-arm-none-eabi-10.3-2021.10-nano_eh-build .
docker run -ti gcc-arm-none-eabi-10.3-2021.10-nano_eh-build
...
sudo tar x -C /usr --strip 1 \
    -f gcc-arm-none-eabi-10.3-2021.10-nano-eh-x86_64-linux.tar.bz2
